...
const roles = ["Fury-EN", "Light-EN"]; //İstediğiniz Rolü Yazabilirsiniz
...
function generateMessages(){
    ...
    for (let role of roles) messages.push(`To Choose Your Side **"${role}"** Click Emoji!`); //DONT CHANGE THIS
    return messages;
}
        ...
        if (msg.author.id == bot.user.id && msg.content != initialMessage){
            ...
            if (user.id != bot.user.id){
                ...
                if (event.t === "MESSAGE_REACTION_ADD"){
                    if (memberObj.removeRole("777177527357407293"))
                    memberObj.addRole(roleObj)
                   client.channels.get("775752340841693225").send(`<@${memberObj.id}> Welcome, send us your questions, suggestions and comments. `);                
                } else {
                  memberObj.removeRole('777177527357407293')         
                }
            }
        }
        })
